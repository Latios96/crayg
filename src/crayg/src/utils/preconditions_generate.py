...
if __name__ == "__main__":
    checks = [
        Check(
            ...
            message='"Pointer should not be null!"',
        ),
        Check(
            name="IS_VALID_INDEX",
            args=["index", "size"],
            condition="(index >= 0 && index < size)",
            message='fmt::format("Index is out of range: {}, size is {}!", index, size)',
        ),
        Check(
            name="EQUALS",
            args=["actual", "expected"],
            condition="actual == expected",
            message='fmt::format("Expected {}, but was {}!", expected, actual)',
        ),
        Check(
            name="NOT_EQUALS",
            args=["actual", "notExpected"],
            condition="actual != notExpected",
            message='fmt::format("Expected {} not to be {}!", actual, notExpected)',
        ),
        Check(
            name="GREATER_OR_EQUALS",
            args=["actual, expected"],
            condition="(actual >= expected)",
            message='fmt::format("Expected {} to be greater or equals than {}!", actual, expected)',
        ),
        Check(
            name="GREATER_THAN",
            args=["actual, expected"],
            condition="(actual > expected)",
            message='fmt::format("Expected {} to be greater than {}!", actual, expected)',
        ),
        Check(
            name="LESS_OR_EQUALS",
            args=["actual, expected"],
            condition="(actual <= expected)",
            message='fmt::format("Expected {} to be less or equals than {}!", actual, expected)',
        ),
        Check(
            name="LESS_THAN",
            args=["actual, expected"],
            condition="(actual < expected)",
            message='fmt::format("Expected {} to be less than {}!", actual, expected)',
        ),
        Check(
            name="WITHIN_RANGE_INCLUSIVE",
            args=["actual, min, max"],
            condition="(actual >= min && actual <= max)",
            message='fmt::format("Expected {} to be in interval [{},{}]!", actual, min, max)',
        ),
        Check(
            name="NOT_WITHIN_RANGE_INCLUSIVE",
            args=["actual, min, max"],
            condition="(actual < min || actual > max)",
            message='fmt::format("Expected {} not to be in interval [{},{}]!", actual, min, max)',
        ),
        Check(
        ...
        Check(
            ...
            message='"Value should not be NaN!"',
        ),
        Check(
            name="HAS_NO_NAN",
            args=["value"],
            condition="(!value.isNan())",
            message='fmt::format("Value {} should not have NaN!", value)',
        ),
        Check(
            name="STATE",
            args=["actualState", "expectedState"],
            condition="(actualState == expectedState)",
            message='fmt::format("Expected state {}, was {}!", actualState, expectedState)',
        ),
        Check(
            name="OPTIONAL_HAS_VALUE",
            args=["optional"],
            condition="(optional.has_value())",
            message='"Expected optional to have value!"',
        ),
        Check(
            name="IS_NORMALIZED_VECTOR",
            args=["vector"],
            condition="(std::fabs((vector).length() - 1) < 0.001f)",
            message='fmt::format("Vector is not unit vector!: {}", vector)',
        ),
    ]
