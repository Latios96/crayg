...
FanTriangulator::FanTriangulator(SubdivisionSurfaceMesh &subdivisionSurfaceMesh)
    : faceVertexIndices(subdivisionSurfaceMesh.faceVertexIndices),
      faceVertexCounts(subdivisionSurfaceMesh.faceVertexCounts) {
}
...
void FanTriangulator::fanTriangulate(std::vector<TriangleMesh::FaceVertexIndices> &target) {
    ...
    for (int faceVertexCount : faceVertexCounts) {
        for (int generatedTriangle = 0; generatedTriangle < (faceVertexCount - 2); generatedTriangle++) {
            target.emplace_back(faceVertexIndices[offset], faceVertexIndices[offset + generatedTriangle + 1],
                                faceVertexIndices[offset + generatedTriangle + 2]);
        }
