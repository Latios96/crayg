...
void UsdMeshReader::translateFaceVaryingUvs(std::shared_ptr<TriangleMesh> &triangleMesh,
    ...
    for (auto id : triangleMesh->faceIds()) {
        const Vector2f v0 = UsdConversions::convert(data[id * 3]);
        const Vector2f v1 = UsdConversions::convert(data[id * 3 + 2]);
        const Vector2f v2 = UsdConversions::convert(data[id * 3 + 1]);
