...
template<typename T>
class TriangleMeshPerFacePrimVar : public TriangleMeshAbstractPrimVar<T> {
 public:
    explicit TriangleMeshPerFacePrimVar(TriangleMesh &triangleMesh) : TriangleMeshAbstractPrimVar<T>(
        triangleMesh) {}
    ...
    friend std::ostream &operator<<(std::ostream &os, const TriangleMeshPerFacePrimVar &var) {
        os << ToStringHelper("TriangleMeshPerFacePrimVar")
            .addMember("faceData", var.faceData)
            .finish();
        return os;
    }
