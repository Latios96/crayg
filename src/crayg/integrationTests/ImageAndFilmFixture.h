...
struct ImageFixture {
    ImageFixture(bool addMetadata = true) : image(Image(128, 72)) {
        createRgbGradientImage(image.rgb);
        if (addMetadata) {
            addTestMetadata();
        }
    }
    ...
    void addCustomChannel(PixelFormat pixelFormat, int colorChannelCount) {
        ...
        createRgbGradientImage(*image.getChannel("custom"));
    }

    Image image;

  private:
    void createRgbGradientImage(PixelBuffer &pixelBuffer) {
...
struct FilmFixture {
    ...
    void addCustomChannel(PixelFormat pixelFormat, int channelCount) {
        ...
        createRgbGradientImage("custom");
    }

    Film film;

  private:
    void createRgbGradientImage(const std::string &channelName) {
