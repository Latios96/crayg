...
#include "sceneIO/read/usd/UsdMaterialTranslationCache.h"
...
TEST_CASE("UsdMaterialTranslationCache::getTranslatedPrimMaterial") {
    ...
    SECTION("no assigned material should translate to default Material") {
        UsdMaterialTranslationCache usdMaterialTranslationCache;

        auto material = usdMaterialTranslationCache.getTranslatedPrimMaterial(geo);
    ...
    SECTION("default material should be cached if no material is assigned") {
        ...
        auto material1 = usdMaterialTranslationCache.getTranslatedPrimMaterial(geo);
        auto material2 = usdMaterialTranslationCache.getTranslatedPrimMaterial(geo);

        REQUIRE(isDefaultMaterial(material1));
        REQUIRE(material1 == material2);
    }

    SECTION("assigned material that has no surface shader should translate to default Material") {
        UsdMaterialTranslationCache usdMaterialTranslationCache;
    ...
    SECTION("assigned material that has not a UsdPreviewSurface as surface shader should translate to default Material") {
        UsdMaterialTranslationCache usdMaterialTranslationCache;
        pxr::UsdShadeMaterialBindingAPI bindingApi(geo.GetPrim());
        bindingApi.Bind(usdShadeMaterial);
    ...
    SECTION("assigned authored UsdPreviewSurface should translate to correct shader") {
        UsdMaterialTranslationCache usdMaterialTranslationCache;
        pxr::UsdShadeMaterialBindingAPI bindingApi(geo.GetPrim());
        bindingApi.Bind(usdShadeMaterial);
