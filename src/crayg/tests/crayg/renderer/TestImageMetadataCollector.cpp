#include "crayg/renderer/ImageMetadataCollector.h"
...
TEST_CASE("TestImageMetadataCollector::collect") {
    ...
    SECTION("should collect realistic camera parameters") {
        ...
        REQUIRE(imageMetadata.read<int>(ImageMetadataTokens::CAMERA_LENS_SURFACE_COUNT) ==
                scene.camera->getLens().surfaces.size());
