...
TEST_CASE("SampleAccumulator::getValue") {
    ...
    SECTION("should return correct accumulated value") {
        auto testData = GENERATE(table<std::vector<Color>, Color>(
            {{{Color::createWhite()}, {Color::createWhite()}},
             {{Color::createBlack(), Color::createWhite()}, {Color::createGrey(0.5f)}},
             {{Color::createGrey(0.1f), Color::createGrey(0.5f), Color::createGrey(0.3f), Color::createGrey(0.9f),
